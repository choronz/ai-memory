...
| `AI_MEMORY_LLM_PROVIDER=gemini` | `gemini-3.1-flash-lite` | Google's hosted option with a generous free tier. ai-memory disables Gemini 3.1 Flash-Lite's default dynamic thinking so hidden thought tokens do not truncate strict JSON. Set `GEMINI_API_KEY` or `GOOGLE_API_KEY`. For higher throughput, set `GEMINI_API_KEYS` (or `GOOGLE_API_KEYS`) with comma-separated keys for round-robin rotation on 429/5xx errors. |
...
| `AI_MEMORY_EMBEDDING_PROVIDER=google` / `gemini` | `gemini-embedding-001` (768-dim) | Google-hosted embeddings via `embedContent`. Set `GEMINI_API_KEY` or `GOOGLE_API_KEY`. For higher throughput, set `GEMINI_API_KEYS` (or `GOOGLE_API_KEYS`) for round-robin rotation on 429/5xx errors. |
