...
> **Two-step hook install pattern.** All three flagship CLIs use the
> same shape: (1) `docker cp` the bundled scripts to your home dir,
> (2) `docker run --rm install-hooks` to render the config snippet.
> `docker cp` works on every docker setup (including rootless and
> userns-remap); a `setup-agent` one-shot exists but has bind-mount
> caveats — see [Bind mounts vs docker cp](#bind-mounts-vs-docker-cp).
...
# pi: prints the "not supported via MCP" explanation
docker run --rm akitaonrails/ai-memory:latest install-mcp --client pi
```

For these MCP-only clients you'll need to nudge the model to call
`memory_query` / `memory_handoff_accept` itself (see the
[CLAUDE.md snippet](../README.md#nudge-the-agent-to-use-memory-proactively)
in the README).
...
The installer supports `--agent codex|opencode|claude-code` and
`--to <dir>`; `--help` prints the full flag list.
